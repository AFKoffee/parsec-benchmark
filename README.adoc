...
CAUTION: The sole purpose of this branch is to enable cross-compilation of _some_ of the programs in this suite to WebAssembly using the https://github.com/arjunr2/WALI[WALI Toolchain]. This required some changes to the codebase that may lead to build failures even for _non-wali configurations_ (like the default 'gcc' configuration) that have worked before.
